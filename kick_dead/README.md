...
# Changelog
### 1.5
- fix silent death sounds during busy scenarios (using CHAN_AUTO can fail to play the sound on the client when several other sounds are playing in dynamic channels, so we instead use static channels)
- both this and hxlib should be updated together due to critical change in a hook used
