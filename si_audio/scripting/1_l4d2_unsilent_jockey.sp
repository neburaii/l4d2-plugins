/*
	...
	---------
		0.9 (Neburai)
			- added TIMER_FLAG_NO_MAPCHANGE flag to the timers created, since player_team event didn't consistently
			fire for every SI before map transitions. I guess it wasn't a problem in versus since this seems like a hard to miss bug
	    0.8 (Sir + J.)
...
void ChangeJockeyTimerStatus(int client, bool bEnable)
{
	...
	if (bEnable) {
		g_hJockeySoundTimer[client] = CreateTimer(g_hJockeyVoiceInterval.FloatValue, delayedJockeySound, client, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
	}
}
